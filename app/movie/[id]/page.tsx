...
async function getMovieDetails(id: IMovieCard["id"]) {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${id}?api_key=${process.env.API_KEY}&language=ru-RU`
  );
  return res.json();
}
...
async function getMovieRec(id: IMovieCard["id"]) {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${id}/recommendations?api_key=${process.env.API_KEY}&language=ru-RU`
  );
  return res.json();
}
...
async function getMovieCasts(id: IMovieCard["id"]) {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${id}/credits?api_key=${process.env.API_KEY}&language=ru-RU`
  );
  return res.json();
}
      ...
      <div className="w-[1200px] max-w-full px-4 mx-auto">
        <div className="flex flex-col mb-6 mt-6">
          <div className="flex justify-between items-center mt-4">
            <h1 className="text-2xl font-medium">В ролях</h1>
            <Link
              href={`/movie/${id}/casts`}
        ...
        <div className="flex flex-col mb-6 mt-6">
          <div className="flex justify-between items-center mt-4">
            <h1 className="text-2xl font-medium">Похожие</h1>
            <Link
